...
import edu.miu.cs.librarysystem.business.BookCopy;
...
/**
 * This class loads data into the data repository and also sets up the storage units that are used
 * in the application. The main method in this class must be run once (and only once) before the
 * rest of the application can work properly. It will create three serialized objects in the
 * dataaccess.storage folder.
 */
public class TestData {
  ...
  public void libraryMemberData() {
    LibraryMember libraryMember =
        new LibraryMember("1001", "Andy", "Rogers", "641-223-2211", addresses.get(4));
    members.add(libraryMember);
    libraryMember = new LibraryMember("1002", "Drew", "Stevens", "702-998-2414", addresses.get(5));
    members.add(libraryMember);

    libraryMember =
        new LibraryMember("1003", "Sarah", "Eagleton", "451-234-8811", addresses.get(6));
    members.add(libraryMember);

    libraryMember =
        new LibraryMember("1004", "Ricardo", "Montalbahn", "641-472-2871", addresses.get(7));
    members.add(libraryMember);
